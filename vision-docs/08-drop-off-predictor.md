...
- **Amazon Comprehend** -- Extracts raw signals per section: key phrase density (DetectKeyPhrases), entity count (DetectEntities), sentiment polarity and shifts (DetectSentiment), and part-of-speech distribution (DetectSyntax). These are raw signals, not readability scores — Bedrock interprets them. Each call is limited to 5KB of text, so process section by section.
- **Amazon Bedrock** -- Interprets the Comprehend signals alongside the text, predicts per-section engagement, identifies drop-off factors, and suggests revisions
- **Amazon S3** -- Stores the input text, metrics, and analysis results

### Data Flow

1. User uploads or pastes a long-form text passage
2. App splits the text into logical sections (by paragraph or heading)
3. App sends each section to Comprehend for key phrase detection (DetectKeyPhrases), entity counting (DetectEntities), sentiment analysis (DetectSentiment), and part-of-speech distribution (DetectSyntax). Each section must be under 5KB.
...
- **S3 bucket** -- Stores input text, Comprehend metrics, and Bedrock analysis
- **In-memory** -- Section splits, metrics, and scores during processing
...
- `comprehend:DetectSyntax` -- Returns part-of-speech tags per token (NOUN, VERB, ADJ, etc.). Useful for token count and POS distribution, but does NOT compute readability scores or sentence complexity metrics. Limited to 5KB per call, 6 languages.
